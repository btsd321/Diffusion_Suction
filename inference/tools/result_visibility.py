...
def test_visualization():
    ...
    np.random.seed(42)
    n_points = 1000
    point_cloud = np.random.randn(n_points, 3) * 0.1
    normals = np.random.randn(n_points, 3)
    normals = normals / np.linalg.norm(normals, axis=1, keepdims=True)  # 归一化
    
    # 生成测试评分
    composite_score = np.random.uniform(0, 1, n_points)
    wrench_score = np.random.uniform(0, 1, n_points)
    feasibility_score = np.random.uniform(0, 1, n_points)
    visibility_score = np.random.uniform(0, 1, n_points)
    
    # 生成测试最佳点
    best_indices = np.argsort(composite_score)[-5:]  # 取前5个最高分
    best_points = []
    for i, idx in enumerate(best_indices):
        best_points.append({
            'rank': i + 1,
            'position': point_cloud[idx].tolist(),
            'normal': normals[idx].tolist(),
            'composite_score': float(composite_score[idx]),
            'original_scores': {
                'wrench_score': float(wrench_score[idx]),
                'feasibility_score': float(feasibility_score[idx]),
                'visibility_score': float(visibility_score[idx])
            }
        })
    ...
    print("测试数据统计:")
    print(f"  - 点云数量: {len(point_cloud)}")
    print(f"  - 最佳点数量: {len(best_points)}")
    print(f"  - 综合评分范围: [{composite_score.min():.4f}, {composite_score.max():.4f}]")
    
    # 测试可视化
    try:
        visualize_results(test_data, 'composite_score', show_best_points=True)
        print("✅ 测试成功完成！")
    except Exception as e:
        print(f"❌ 测试失败: {e}")
        raise
...
def main():
    ...
    if len(sys.argv) != 3:
        print("使用方法: python result_visibility.py <point_cloud_file> <best_points_json>")
        print("示例: python result_visibility.py point_cloud.txt best_points.json")
        sys.exit(1)
    
    point_cloud_file = sys.argv[1]
    best_points_file = sys.argv[2]
    
    try:
        visualize_results(point_cloud_file, best_points_file)
    except Exception as e:
        print(f"错误: {e}")
        sys.exit(1)
